...
def creartabla(nombre,columnas,proyecto):
    ...
    restricciones.append('ALTER TABLE '+nombre+' ADD CONSTRAINT CPK_'+nombre+ ' PRIMARY KEY(PK_'+nombre+");\r\n")
    for hijo in proyecto:
        if hijo['idnodo'] in columnas:
            if hijo['caracteristica'] == 'Hoja':
                ...
                restricciones.append('ALTER TABLE '+nombre+' ADD CONSTRAINT FK_'+hijo['tiponodo']+ ' FOREIGN KEY('+'K_'+hijo['tiponodo']+') REFERENCES '+hijo['tiponodo']+'(PK_'+hijo['tiponodo']+");\r\n")
