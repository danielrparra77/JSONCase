...
def guardarendb(request):
    ...
    try:
        ...
        if not models.Proyecto.objects.filter(K_NombreProyecto=proyecto["nombre"]).exists():
...
@login_required()
@csrf_exempt
def sobreescribirproyecto(request):
    """
        En esta vista se propone sobreescribir proyectos guardados en la base de datos
    """
    try:
        proyecto = request.POST.get('proyecto','')
        print request.POST
        proyecto = json.loads(proyecto)
        mproyecto = models.Proyecto.objects.filter(K_NombreProyecto=proyecto["nombre"])
        mproyecto = mproyecto[0]
        mproyecto.K_UsuarioCreo=proyecto["usuario"]
        mproyecto.V_FechaCreacion=datetime.now()
        mproyecto.save()
        print proyecto["estilo"]
        estilo = json.loads(proyecto["estilo"])
        tipoestilo = ["fondonodo","colorlineanodo","grosornodo","fondohoja","colorlineahoja",
            "grosorhoja","fuenteletra","colorfuente","tamanofuente","grosorconexion","colorconexion"]
        it = 0
        for estilodb in models.EstiloObjeto.objects.filter(K_Proyecto=mproyecto):
            estilodb.V_Estilo=tipoestilo[it]
            estilodb.V_Valor=estilo[tipoestilo[it]]
            estilodb.save()
            it = it +1
        print estilo["fondonodo"]
        nodos = json.loads(proyecto["proyecto"])
        ...
        mvalor = []
        #Se crearan todos los nodos
        nodosguardados = models.Objeto.objects.filter(K_Proyecto=mproyecto)
        for nododb in models.Objeto.objects.filter(K_Proyecto=mproyecto):
            models.ValorObjeto.objects.filter(K_Objeto=nododb).delete()
        nodosguardados.delete()
        for nodo in nodos:
            print nodo
            nuevo = models.Objeto(V_IdLocal=nodo["idnodo"],V_CoordenadaX=nodo["x"],V_CoordenadaY=nodo["y"],N_ClaseObjeto=nodo["caracteristica"],
            N_SiRaiz=(nodo["padre"]==''),K_Proyecto=mproyecto,K_TipoObjeto=nodo["tiponodo"])     
